...
### Phase 2b: Google provider implementation and token encryption helpers
...
- [ ] Token encryption helpers at `src/lib/calendar/crypto.ts` (or similar): AES-256-GCM with a key from `TOKEN_ENCRYPTION_KEY` (32 random bytes, base64). Versioned ciphertext format `v1:base64nonce:base64ciphertext`.
- [ ] Side-effecting `src/lib/calendar/providers/register-all.ts` that imports `google.ts` and calls `registerProvider`.
- [ ] `googleapis` added to dependencies.
- [ ] `TOKEN_ENCRYPTION_KEY` added to `.env.example` with generation instructions; production value provisioned in Bitwarden and on the droplet `.env`.
- [ ] Unit tests for the encryption helpers (round-trip, tampering detection, version prefix handling). No tests against live Google in 2b; provider tests stub the SDK.

### Phase 2c: OAuth routes, connect/disconnect actions, `/settings/calendars` page

- [ ] Google Cloud Console OAuth client provisioned (manual ops; documented in a Phase 2c PR body or a new `docs/ops/google-oauth.md` if it warrants permanence). Authorized redirect URIs: `https://cal.noclulabs.com/api/calendar/google/callback` and `http://localhost:3000/api/calendar/google/callback`. Client id and secret stored in Bitwarden and on the droplet `.env` as `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`.
- [ ] `GET /api/calendar/google/connect` route: signed state, redirect to `buildAuthorizationUrl`.
- [ ] `GET /api/calendar/google/callback` route: validate state, call `exchangeCode`, encrypt tokens, upsert `calendar_connections` row (DELETE-then-INSERT to honor the unique constraint cleanly), redirect to `/settings/calendars`.
...
- [ ] Refresh token handling: a thin wrapper that calls `refreshAccessToken` when `tokenExpiresAt` is within a 60s safety margin of now, re-encrypts and persists the new token set. On refresh failure, DELETE the row and surface a "reconnect required" UI state.
