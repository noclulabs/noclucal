...
- Phase 5c (wire the confirmation through a queued job): complete (2026-06-12). The branded confirmation email now goes out in production: `confirmBooking` appends a best-effort enqueue after the claim and the best-effort Google write-back (the confirm order is unchanged; an enqueue failure is logged and never affects the booking), and the worker consumes the `send-confirmation` job and sends through Resend. The job payload is self-contained (exactly the `sendConfirmationEmail` input, re-exported as `SendConfirmationJobPayload`), so the worker does no database read and no schema changed. Job options on the notifications queue: 3 attempts with exponential backoff, `removeOnComplete: true` (payloads carry invitee PII), `removeOnFail: 100`. `server-only` was removed from the email path and dropped as a dependency (the tsx worker imports the send path and the marker throws in plain Node; `INFRA-PLAYBOOK.md` § The worker-import constraint); the send now carries a plain-text body alongside the HTML, and the Meet link is omitted from the payload when the Google write-back failed (the template already rendered gracefully without it). Required ops step on merge: verify the sending domain in Resend and set `RESEND_API_KEY` and `EMAIL_FROM` in the droplet `/opt/noclucal/.env`; a missing value fails sends gracefully (retried, then logged) and never breaks bookings. Bounce and complaint handling stays deferred (see Phase 5c below). 5d and 5e remain.
...
### Phase 5c: wire the confirmation through a queued job (complete)

- [x] Enqueue a confirmation job from `confirmBooking` and send the branded email from the worker, replacing reliance on Google's own invite as the only confirmation. The enqueue is best-effort and appended after the claim and the Google write-back; the confirm order is unchanged. The `SEND_CONFIRMATION` job payload is self-contained (exactly the `sendConfirmationEmail` input), so the worker does no database read and no schema changed; job options are 3 attempts with exponential backoff, `removeOnComplete: true` (invitee PII), and a bounded `removeOnFail`.
- [x] The wiring constraint resolved by taking `server-only` off the send path (the marker throws in the plain-Node tsx worker, and `--conditions react-server` would break `react-dom/server`, which `@react-email/render` uses); the dependency is dropped and the email modules are server-side by convention, like the DB and crypto modules. Rationale in `INFRA-PLAYBOOK.md` § The worker-import constraint.
- [x] The send now produces a plain-text body alongside the HTML for deliverability, and the payload's Meet link is optional (absent when the Google write-back failed; the template renders gracefully without it).
- [ ] Bounce and complaint handling via Resend webhooks remains a logged, deferred item (descoped from 5b, and again from 5c: the wiring PR sends, it does not process delivery feedback). Likely 5e or a standalone item.
- The 5b planning question (should the booking record persist the invitee timezone) resolved itself: the row has carried `invitee_timezone` since 4a and the Google event id / Meet link refs since 4d, and the self-contained 5c payload reads none of them back. Sending notification emails from the stored booking record (using those columns) is the Phase 6 concern, when reschedule and cancel fire later from the record rather than within the booking request.
