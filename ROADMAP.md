...
- [ ] Front-door routing that ties the public flow together (the public profile `/[username]` listing a user's bookable event types) and the host-facing copy-link affordance for a booking page.
- [ ] The `username` unique migration (the public resolver assumes `username` is unique; the column is not yet constrained), plus the polish pass. (Branded email confirmations and reminders remain Phase 5.)
